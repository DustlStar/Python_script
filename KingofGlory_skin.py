...
# 皮肤图片存放路径
def make_path():
    Imgs_path = r'E:\\photo\\skin\\'
    if not os.path.exists(Imgs_path):
        os.makedirs(Imgs_path)
    return Imgs_path

# 定义一个函数，用于下载并存储图片
def save_IMG(Imgs_path):
    num = 0
    # 逐一遍历英雄
    for i in hero_number:
        ...
        for sk_num in range(15):
            # 英雄皮肤的URL链接
            heroskin_links = 'http://game.gtimg.cn/images/yxzj/img201606/skin/hero-info/' + str(i) + '/' + str(
                i) + '-bigskin-' + str(sk_num) + '.jpg'
            hl = requests.get(heroskin_links)
            if hl.status_code == 200:
                # 将图片保存下来，并以"英雄名称_皮肤序号"方式命名
                with open(Imgs_path + hero_name[num] + str(sk_num) + '.jpg', 'wb') as f:
                    f.write(hl.content)
        num = num + 1
